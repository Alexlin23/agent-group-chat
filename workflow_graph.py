...
from workflow_nodes import NODE_HANDLERS
...
class GraphState(TypedDict, total=False):
    ...
    completed_nodes: Annotated[list[str], operator.add]  # 已完成节点，resume 时跳过
...
def _make_node_func(node_def: dict):
    ...
    Dispatches to the appropriate handler via NODE_HANDLERS registry.
    """
    node_id = node_def["id"]
    ...
    async def _node_func(state: GraphState) -> dict:
        ...
        if handler is None:
            return {"status": "failed", "error": f"Unknown node type: '{node_type}' for node '{node_id}'"}
        return await handler(state, ctx, node_id, node_def)
...
def _make_condition_func(cond_def: dict):
    ...
    async def _cond_func(state: GraphState) -> dict:
        ...
        cond_node_def = {"condition": condition, "paths": paths, "name": f"cond_{node_id}"}
        return await NODE_HANDLERS["condition"](
            state=state, ctx=ctx, node_id=f"cond_{node_id}", node_def=cond_node_def,
        )
