"""Workflow node functions for LangGraph engine.

Each node type is a function with uniform signature:
    async def xxx_node(state, ctx, node_id, node_def) -> dict

All node functions are registered in NODE_HANDLERS dict for dispatch.
To add a new node type: write the function, add one entry to NODE_HANDLERS.

Node types:
- agent: Call an agent via Hermes API
- condition: Use LLM to decide which branch to take
- parallel: Fan-out multiple agent calls, fan-in results
- human: Pause for human input
- code: Execute Python code
"""

import asyncio
import json
import re
from datetime import datetime
from typing import Any

from hermes_client import stream_hermes
from text_utils import build_context_text, strip_agent_prefix
from workflow_state import WorkflowState, WorkflowContext


# ── Skill 缓存 ──

_SKILL_CACHE = None


def _load_project_skill() -> str:
    """Load the project skill file (cached)."""
    global _SKILL_CACHE
    if _SKILL_CACHE is not None:
        return _SKILL_CACHE
    from pathlib import Path
    skill_path = Path(__file__).parent / ".hermes" / "skills" / "agent-group-chat.md"
    if skill_path.exists():
        _SKILL_CACHE = skill_path.read_text(encoding="utf-8") + "\n\n"
    else:
        _SKILL_CACHE = ""
    return _SKILL_CACHE


# ── 模板渲染 ──

def render_template(template: str, variables: dict[str, Any]) -> str:
    """Render a {{var}} template with variables."""
    def replacer(match):
        var_name = match.group(1).strip()
        val = variables.get(var_name, match.group(0))
        return str(val) if not isinstance(val, str) else val
    return re.sub(r'\{\{(.+?)\}\}', replacer, template)


# ── Agent 节点 ──

async def agent_node(
    state: WorkflowState,
    ctx: WorkflowContext,
    node_id: str,
    node_def: dict,
) -> dict:
    """Execute an agent node — call Hermes API and return the response."""
    agent_id = node_def.get("agent_id", "")
    prompt_template = node_def.get("prompt", "")
    output_var = node_def.get("output_var", "")
    node_name = node_def.get("name", node_id)

    agents = ctx["agents_ref"]
    event_buffer = ctx.get("event_buffer")
    agent = agents.get(agent_id)

    if not agent:
        return {
            "status": "failed",
            "completed_nodes": [node_id],
            "error": f"Agent '{agent_id}' not found for node '{node_id}'",
        }

    rendered_prompt = render_template(prompt_template, state.get("variables", {}))

    if event_buffer:
        event_buffer.push("node_start", {
            "node_id": node_id, "node_name": node_name,
            "agent_id": agent_id, "agent_name": agent["name"],
        })

    context_text = build_context_text(state.get("messages", []), agents)
    skill_text = _load_project_skill()
    system_prompt = (
        f"{agent.get('system_prompt', '')}\n\n"
        f"{skill_text}"
        f"---\n工作流节点: {node_name}\n---\n\n"
        f"{rendered_prompt}"
    )

    full_response = ""
    try:
        async for chunk in stream_hermes(ctx["hermes_url"], ctx["hermes_key"], system_prompt):
            full_response += chunk
            if event_buffer:
                event_buffer.push("node_text", {"node_id": node_id, "agent_id": agent_id, "text": chunk})
    except RuntimeError as e:
        if event_buffer:
            event_buffer.push("node_error", {"node_id": node_id, "agent_id": agent_id, "error": str(e)})
        return {"status": "failed", "completed_nodes": [node_id], "error": f"Node '{node_id}' failed: {str(e)[:200]}"}

    clean_response = strip_agent_prefix(full_response, agent["name"])

    if event_buffer:
        event_buffer.push("node_done", {
            "node_id": node_id, "node_name": node_name,
            "agent_id": agent_id, "agent_name": agent["name"],
            "output_var": output_var, "full_response": clean_response,
        })

    variables = dict(state.get("variables", {}))
    node_outputs = dict(state.get("node_outputs", {}))
    if output_var:
        variables[output_var] = clean_response
    node_outputs[node_id] = clean_response

    return {
        "current_node": node_id,
        "completed_nodes": [node_id],
        "variables": variables,
        "node_outputs": node_outputs,
        "messages": [{"role": "assistant", "content": clean_response, "agent_id": agent_id, "node_id": node_id, "timestamp": datetime.now().isoformat()}],
        "execution_log": [{"node_id": node_id, "agent_id": agent_id, "status": "completed", "timestamp": datetime.now().isoformat()}],
    }


# ── 条件节点 ──

async def condition_node(
    state: WorkflowState,
    ctx: WorkflowContext,
    node_id: str,
    node_def: dict,
) -> dict:
    """Execute a condition node — ask LLM which branch to take."""
    condition_prompt = node_def.get("condition", "")
    paths = node_def.get("paths", [])
    node_name = node_def.get("name", node_id)

    event_buffer = ctx.get("event_buffer")

    if event_buffer:
        event_buffer.push("condition_start", {"node_id": node_id, "node_name": node_name, "paths": paths})

    rendered = render_template(condition_prompt, state.get("variables", {}))
    system_prompt = (
        f"你是一个工作流条件判断器。根据以下信息决定走哪条路径。\n\n"
        f"可用路径：{', '.join(paths)}\n\n"
        f"你必须只输出一个路径名，不要输出其他内容。\n\n"
        f"---\n{rendered}"
    )

    from hermes_client import call_hermes
    try:
        response = await call_hermes(ctx["hermes_url"], ctx["hermes_key"], [{"role": "system", "content": system_prompt}], temperature=0.1, max_tokens=50)
    except Exception as e:
        return {"status": "failed", "completed_nodes": [node_id], "error": f"Condition node '{node_id}' failed: {str(e)[:200]}"}

    chosen = response.strip().lower()
    matched = None
    for path in paths:
        if path.lower() in chosen:
            matched = path
            break

    if not matched:
        matched = paths[0] if paths else ""
        if event_buffer:
            event_buffer.push("condition_fallback", {"node_id": node_id, "raw_response": response[:100], "fallback_to": matched})

    if event_buffer:
        event_buffer.push("condition_done", {"node_id": node_id, "node_name": node_name, "chosen_path": matched})

    return {
        "current_node": node_id,
        "completed_nodes": [node_id],
        "conditional_result": matched,
        "execution_log": [{"node_id": node_id, "status": "condition_resolved", "chosen_path": matched, "timestamp": datetime.now().isoformat()}],
    }


# ── 并行节点 ──

async def parallel_node(
    state: WorkflowState,
    ctx: WorkflowContext,
    node_id: str,
    node_def: dict,
) -> dict:
    """Execute an agent node in parallel for each item in a list."""
    agent_id = node_def.get("agent_id", "")
    prompt_template = node_def.get("prompt", "")
    items_var = node_def.get("items_var", "items")
    item_var = node_def.get("item_var", "item")
    output_var = node_def.get("output_var", "")
    node_name = node_def.get("name", node_id)

    event_buffer = ctx.get("event_buffer")

    raw_items = state.get("variables", {}).get(items_var, "")
    if isinstance(raw_items, list):
        items = raw_items
    elif isinstance(raw_items, str):
        try:
            items = json.loads(raw_items)
            if not isinstance(items, list):
                items = [raw_items]
        except (json.JSONDecodeError, ValueError):
            items = [line.strip() for line in raw_items.split("\n") if line.strip()]
    else:
        items = [raw_items]

    if not items:
        return {
            "current_node": node_id, "completed_nodes": [node_id],
            "variables": {**state.get("variables", {}), output_var: "[]"},
            "execution_log": [{"node_id": node_id, "status": "skipped", "reason": "empty items list", "timestamp": datetime.now().isoformat()}],
        }

    if event_buffer:
        event_buffer.push("parallel_start", {"node_id": node_id, "node_name": node_name, "item_count": len(items)})

    async def _run_one(idx: int, item: Any) -> dict:
        item_vars = dict(state.get("variables", {}))
        item_vars[item_var] = str(item)
        sub_state = {**state, "variables": item_vars}
        sub_def = {**node_def, "id": f"{node_id}[{idx}]", "name": f"{node_name}[{idx}]", "output_var": ""}
        return await agent_node(state=sub_state, ctx=ctx, node_id=f"{node_id}[{idx}]", node_def=sub_def)

    results = await asyncio.gather(*[_run_one(i, item) for i, item in enumerate(items)], return_exceptions=True)

    outputs, all_logs, all_messages = [], [], []
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            outputs.append(f"ERROR: {str(result)[:200]}")
            all_logs.append({"node_id": f"{node_id}[{i}]", "status": "failed", "error": str(result)[:200], "timestamp": datetime.now().isoformat()})
        elif isinstance(result, dict):
            if result.get("status") == "failed":
                outputs.append(f"ERROR: {result.get('error', 'unknown')}")
            else:
                node_out = result.get("node_outputs", {})
                outputs.append(node_out.get(f"{node_id}[{i}]", ""))
            all_logs.extend(result.get("execution_log", []))
            all_messages.extend(result.get("messages", []))

    if event_buffer:
        event_buffer.push("parallel_done", {"node_id": node_id, "node_name": node_name, "completed": len([o for o in outputs if not str(o).startswith("ERROR:")]), "failed": len([o for o in outputs if str(o).startswith("ERROR:")])})

    variables = dict(state.get("variables", {}))
    variables[output_var] = json.dumps(outputs, ensure_ascii=False)
    node_outputs = dict(state.get("node_outputs", {}))
    node_outputs[node_id] = outputs

    return {
        "current_node": node_id, "completed_nodes": [node_id],
        "variables": variables, "node_outputs": node_outputs,
        "messages": all_messages, "execution_log": all_logs,
    }


# ── 人类介入节点 ──

async def human_node(
    state: WorkflowState,
    ctx: WorkflowContext,
    node_id: str,
    node_def: dict,
) -> dict:
    """Pause workflow and wait for human input."""
    prompt = node_def.get("prompt", "")
    output_var = node_def.get("output_var", "human_response")
    node_name = node_def.get("name", node_id)

    event_buffer = ctx.get("event_buffer")
    rendered = render_template(prompt, state.get("variables", {}))

    if event_buffer:
        event_buffer.push("human_input_required", {"node_id": node_id, "node_name": node_name, "prompt": rendered})

    human_input = state.get("human_input", "")
    if human_input and human_input.strip():
        variables = dict(state.get("variables", {}))
        variables[output_var] = human_input
        if event_buffer:
            event_buffer.push("human_input_received", {"node_id": node_id, "input": human_input[:200]})
        return {
            "current_node": node_id, "completed_nodes": [node_id],
            "variables": variables, "waiting_for_human": False, "human_input": "",
            "execution_log": [{"node_id": node_id, "status": "human_input_received", "timestamp": datetime.now().isoformat()}],
        }

    return {
        "current_node": node_id, "status": "paused",
        "waiting_for_human": True, "human_prompt": rendered,
        "execution_log": [{"node_id": node_id, "status": "waiting_for_human", "timestamp": datetime.now().isoformat()}],
    }


# ── Code 节点 ──

async def code_node(
    state: WorkflowState,
    ctx: WorkflowContext,
    node_id: str,
    node_def: dict,
) -> dict:
    """Execute Python code with variable injection."""
    from code_executor import execute_code

    code = node_def.get("code", "")
    output_var = node_def.get("output_var", "")
    timeout = node_def.get("timeout", 60)
    node_name = node_def.get("name", node_id)

    event_buffer = ctx.get("event_buffer")

    if event_buffer:
        event_buffer.push("node_start", {"node_id": node_id, "node_name": node_name, "type": "code"})

    if not code.strip():
        return {"status": "failed", "completed_nodes": [node_id], "error": f"Node '{node_id}': empty code"}

    variables = dict(state.get("variables", {}))

    # 在线程池中执行，带超时
    try:
        loop = asyncio.get_event_loop()
        result = await asyncio.wait_for(
            loop.run_in_executor(None, execute_code, code, variables),
            timeout=timeout,
        )
    except asyncio.TimeoutError:
        return {"status": "failed", "completed_nodes": [node_id], "error": f"Node '{node_id}': code execution timed out ({timeout}s)"}

    if result.get("error"):
        if event_buffer:
            event_buffer.push("node_error", {"node_id": node_id, "error": result["error"][:200]})
        return {
            "status": "failed", "completed_nodes": [node_id],
            "error": f"Node '{node_id}' code error: {result['error'][:200]}",
            "execution_log": [{"node_id": node_id, "status": "failed", "error": result["error"][:200], "timestamp": datetime.now().isoformat()}],
        }

    # 变量已经被 execute_code in-place 修改
    node_outputs = dict(state.get("node_outputs", {}))
    output_value = result.get("result", result.get("stdout", ""))
    if output_var:
        variables[output_var] = str(output_value) if not isinstance(output_value, str) else output_value
    node_outputs[node_id] = output_value

    if event_buffer:
        event_buffer.push("node_done", {"node_id": node_id, "node_name": node_name, "type": "code", "output_var": output_var})

    return {
        "current_node": node_id, "completed_nodes": [node_id],
        "variables": variables, "node_outputs": node_outputs,
        "execution_log": [{"node_id": node_id, "status": "completed", "timestamp": datetime.now().isoformat()}],
    }


# ── 注册表 ──

NODE_HANDLERS = {
    "agent": agent_node,
    "condition": condition_node,
    "parallel": parallel_node,
    "human": human_node,
    "code": code_node,
}
